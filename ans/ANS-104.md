...
- Add tags to the DataItem as described by the specification in [section 1.1](#1.1-transaction-tags)
- Provide a binary payload for the DataItem matching the Bundle Transaction Body Format described in [section 1.2](#1.2-transaction-body-format), i.e. the Bundle header outlining the count, size, and IDs of the subsequent, nested DataItems, each of which should be verifiable using the method described in [section 2.1](#2.1-verifying-a-dataitem).
